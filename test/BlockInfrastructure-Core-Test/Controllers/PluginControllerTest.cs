...
public class PluginControllerTest(ContainerFixture containerFixture) : IntegrationsTestHelper(containerFixture)
{
    ...
    [Fact(DisplayName = "GET /channels/{channelId}/plugins/installed: ListInstalledPlugins는 인증하지 않은 사용자에 대해 401을 반환합니다.")]
    public async Task Is_ListInstalledPlugins_Returns_401_For_Unauthorized_User()
    {
        // Let
        var (user, token) = await CreateAccountAsync();
        var channel = await CreateChannelAsync(token.Token);
        ...
        // Do
        WebRequestClient.DefaultRequestHeaders.Clear();
        var response = await WebRequestClient.GetAsync($"/channels/{channel.Id}/plugins/installed");
    ...
    [Fact(DisplayName =
        "GET /channels/{channelId}/plugins/installed: ListInstalledPlugins는 만약 허용되지 않은 사용자가 요청한 경우 403 Forbidden을 반환합니다.")]
    public async Task Is_ListInstalledPlugins_Returns_403_For_User_Without_Proper_Permission()
    {
        // Let
        var (user, token) = await CreateAccountAsync();
        var channel = await CreateChannelAsync(token.Token);
    ...
    [Fact(DisplayName = "GET /channels/{channelId}/plugins/installed: ListInstalledPlugins는 인증된 사용자에 대해 200을 반환합니다.")]
    public async Task Is_ListInstalledPlugins_Returns_200_For_Authorized_User()
    {
        // Let
        var (user, token) = await CreateAccountAsync();
        var channel = await CreateChannelAsync(token.Token);

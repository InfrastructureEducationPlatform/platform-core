...
class GlobalExceptionHandler {
    ...
    fun handleApiException(apiException: ApiException): ResponseEntity<ErrorResponse> {
        val errorResponse = ErrorResponse(apiException.statusCode, apiException.errorMessage, apiException.errorTitle)
    ...
    fun handleGlobalException(exception: Exception): ResponseEntity<ErrorResponse> {
        return ResponseEntity<ErrorResponse>(ErrorResponse(HttpStatus.INTERNAL_SERVER_ERROR, "Unknown Error occurred", ErrorTitle.UNKNOWN_INTERNAL_ERROR), HttpStatus.INTERNAL_SERVER_ERROR)
    }
}